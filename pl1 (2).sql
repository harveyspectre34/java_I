...
begin
	declare exit handler for 1062 select "invalie data" into @b;
	select "Record Inserted by saleel" into @x;
end $
delimiter ;
...
CREATE trigger tr1 after insert on dept for each row  
begin
	insert into bkdept values(new.deptno, new.dname, new.loc, new.pwd, new.startedon, curdate(), curtime(), concat('The new record was inserted by ', user()));
end $
delimiter ;
...
begin
	declare x int;
	select true into x from bkdept where deptno = old.deptno;
	if x then
		signal sqlstate '42000' set message_text='ABABABABABABABAB';
	else
		insert into bkdept values(old.deptno, old.dname, old.loc, old.pwd, old.startedon, curdate(), curtime(), concat('The old record from DEPT relation was inserted by ', user()));
	end if;
end $
delimiter ;
...
DROP trigger IF EXISTS tr1;
delimiter $
CREATE trigger tr1 before insert on r1 for each row  
begin
	insert into r2 values(new.c1);
end $
delimiter ;

mysql> insert into r1 values (default);

-----------------------------------------------------

DROP trigger IF EXISTS tr1;
delimiter $
CREATE trigger tr1 after insert on r1 for each row  
begin
	insert into r2 values(new.c1);
end $
delimiter ;
...
DROP TRIGGER IF EXISTS tr1;
delimiter $
CREATE TRIGGER tr1 BEFORE INSERT ON f FOR EACH ROW
begin
  declare x INT;
  SELECT COUNT(*) INTO x FROM f;
  if x > 3 then
...
DROP TRIGGER IF EXISTS tr1;
delimiter $
CREATE TRIGGER tr1 after INSERT ON f1 FOR EACH ROW
begin
  declare x INT;
  SELECT COUNT(*) INTO x FROM f1;
  if x > 3 then
     signal sqlstate '42000' set message_text = 'error by SALEEL'; 
  end if;
end $
delimiter ;
...
DROP TRIGGER IF EXISTS tr1;
delimiter $
CREATE TRIGGER tr1 BEFORE INSERT ON f1 FOR EACH ROW
begin
	SET NEW.C1 := UPPER(NEW.C1);
end $
delimiter ;
